...
public class Kakao2020_블록이동하기 {

	static class Solution{

		static class Node{
			int y,x,d,y2,x2,distance; //(y,x)~(y2,x2) 블록, distance: (0,0)에서 거리

			public Node(int y, int x, int d, int distance) { // d=0 가로, d=1 세로
				this.y=y;
				this.x=x;
				this.d=d;
		...
		boolean 빈칸(int y, int x) {
			...
			return board[y][x]==0; // 이미 거친곳이면 false, 아니면 true
		}

		Node 이동(Node node, int i) { // 오른쪽, 왼쪽, 위, 아래 이동
		...
		public int solution(int [][] board) {
			...
			while(queue.size()>0) {
				...
				for(int i=0;i<4;++i) {
					...
					if(visited[node2.y][node2.x][node.d]) continue;
					queue.add(node2);
				}
				for(int i=0;i<8;++i) {
